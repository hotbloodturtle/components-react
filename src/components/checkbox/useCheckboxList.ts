...
const useCheckboxList = ({ initialNames }: { initialNames: string[] }) => {
  const [checkedAll, setCheckedAll] = useState(false)
  const [checkboxList, setCheckboxList] = useState<CheckboxProps[]>([])

  useEffect(() => {
    setCheckboxList(initialNames.map((name: string) => ({ id: name, checked: false })))
  }, [])
  ...
  const changeCheckedList = (id: string, checked: boolean) => {
    setCheckboxList(checkboxList.map((item) => (item.id === id ? { id, checked } : item)))
  }
